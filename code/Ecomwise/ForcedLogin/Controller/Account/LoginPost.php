...
namespace Ecomwise\ForcedLogin\Controller\Account;
...
class LoginPost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
     ...
     * @param \Magento\Customer\Model\Customer $customerData
    ...
    public function allowedCustomerGroups(){
        return $this->forcedLoginHelper->getAllowedCustomerGroups();
    }
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
                    ...
                    $customerData = $this->customerData->setWebsiteId($websiteId)->loadByEmail($login['username']);
                    $customerGroupId = $customerData->getGroupId();
                    $allowedCustomerGroups = $this->allowedCustomerGroups();
                    $allowedCustomerGroupsArray = explode(",", $allowedCustomerGroups);

                    $allowedLogin = false;
                    if (in_array($customerGroupId, $allowedCustomerGroupsArray)) {
                        $allowedLogin = true;
                    }
                    
                    if($allowedLogin == false){
                        $emailSender = $this->forcedLoginHelper->getCustomerSupportSenderEmail();
                        $this->messageManager->addComplexErrorMessage('ecwcustomergroups',
                        [
                            'url' => 'mailto:' . $emailSender,
                        ]);
