...
class StoreCreditRepository implements StoreCreditRepositoryInterface
{
    ...
    /**
     * @param float $amount
     * @return StoreCreditRepositoryInterface|string
    ...
    private function getTotalsAmount($quote)
    {
        ...
        foreach ($totals as $code => $total) {
            if ($code == 'subtotal' || $code == 'tax') {
                $totals_amount += $total->getValue();
            }
        }
        $totals_amount += $shippingAmount;
        $convert_to_base_currency = $this->bssStoreCreditHelper->convertBaseFromCurrency($totals_amount);
        return $this->priceCurrency->round($convert_to_base_currency);
    }
