...
class MassActive extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $item) {
            ...
            $this->activePromotionBar($item);
            $active++;
        }

        $this->messageManager->addSuccessMessage(__('A total of %1 record(s) have been actived.', $active));
     ...
     * @throws \Exception
     */
    protected function activePromotionBar($promotionBar)
    {
        try {
            $promotionBar->setStatus(1);
            $promotionBar->save();
        } catch (\Exception $e) {
